...
def _build_dashboard_template() -> str:
    """Return a self-contained 32-row dashboard HTML template.

    Returns:
        HTML text with inline JSON placeholders for the metrics and row data.
    """
...
<title>Replay Cap-1m 32-Row Dashboard</title>
<style>
  body {
...
<body>
<main>
  <h1>Replay Cap-1m 32-Row Dashboard</h1>
  <p>Focused dashboard for the capped 32-row rerun.</p>
  <p>
    This preview renders the runtime-only replay artifact
    <code>data/replay_cap1m_results_32rows.parquet</code>. It covers the full
    32-row rerun used for the one-minute-cap pass.
  </p>
  <section class="card">
    <h2>Embedded preview payloads</h2>
    <p>
      The standalone HTML writes the same JSON payloads that would otherwise be
      saved to <code>replay_cap1m_32rows_metrics.json</code> and
      <code>replay_cap1m_32rows_rows.json</code>.
    </p>
  </section>
</main>
...
<script id="basepagesRowsData" type="application/json">
[]
</script>
</body>
</html>
