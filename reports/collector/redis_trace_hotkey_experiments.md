...
2. Capture a YCSB-backed trace with `python/kernmlops/replay/capture_redis_trace.sh`.
...
This report is the higher-level explanation of why the workflow looks the way
it does and what the experiments showed.
...
The second calibration hit the requested band, so `python/kernmlops/replay/capture_redis_trace.sh`
